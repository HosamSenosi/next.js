...
mod test {
    ...
    fn should_properly_handle_intercept_routes() {
        // This is here because it would match exactly what webpack currently exports. It needs
        // further testing to determine whether we should be matching the webpack
        // implementation exactly or not
        // let webpack_result =
        //     "^/(?<locale>[^/]+?)/example/\\(\\.\\.\\.\\)(?<locale>[^/]+?)/intercepted?)";
        let expected = "^/(?P<nxtPlocale>[^/]+?)/example/\\(\\.\\.\\.\\)(?P<nxtIlocale>[^/]+?)/\
                        intercepted(?:/)?$";
        let intercept_route =
            get_named_middleware_regex("/[locale]/example/(...)[locale]/intercepted");
        assert_eq!(intercept_route, expected)
    }
}
